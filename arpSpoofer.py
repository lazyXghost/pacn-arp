...
class ARPSpoofer:
    ...
    def scan_network(self):
        ...
        for client in sorted_data:
            try:
                mac_resolved = self.mac_resolver[client["MAC"][:8].replace(":","-").upper()]
    ...
    def spoof_mac(self, client_ip, spoof_source=False):
        ...
        if not self.is_client_present(client_ip):
            raise Scapy_Exception(
                f'Client IP {client_ip} not present on network')
            
        self.spoofed_clients.append({"original_IP": client_ip,"original_MAC": self.get_mac_by_ip(client_ip)})
    ...
    def is_client_present(self, client_ip):
        icmp_packet = IP(dst=client_ip, ) / ICMP()
    ...
    def get_host_bits(self, subnet_mask):
        ...
        binary_mask = ''.join(
            [bin(int(octet))[2:].zfill(8) for octet in octets])
        host_bits = 0
